...
## 2026-04-22 両重换算规则
- 单件入库单头新增 `convertLiangWeight`，由用户在表头勾选“显示両重”决定是否换算，默认不换算。
- 换算系数固定为 `37.429`，后端保存时统一按 `両重 = 金重 / 37.429` 计算，保留三位小数，按第四位四舍五入。
- 未勾选换算时，入库明细 `liangWeight` 保持为空，不写 `0`，也不使用金重兜底。
- 入库审核生成 `wh_item` 时，`liangWeight` 随明细复制到单件，供后续打印任务读取。
